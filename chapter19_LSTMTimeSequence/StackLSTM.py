...
def build_model():
    model = Sequential()
    model.add(LSTM(units=4, input_shape=(BATCH_SIZE, look_back,1),return_sequences=True,stateful=True))
    model.add(LSTM(units=4, input_shape=(BATCH_SIZE, look_back, 1),stateful=True))
